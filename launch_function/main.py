...
def main():
    """
    Select the function to launch (uncomment the corresponding line).
    ...
    You can use argument apz_with_tc=True to consider than the default IXL approach zone is the Track Circuit containing
     ...
     physical block by default,
     else the tool considers the IVB upstream the signal, which is the default case if no precision is given.
    You can use as optional input an Excel file defining the IXL Approach Zone, it can only contain the signals where
     ...
     IVB (or TC) upstream the signal. Indeed, it can explicitly said in the ZC-IXL ICDD that for specified signals the
     APZ is extended to specified IVB/TC.

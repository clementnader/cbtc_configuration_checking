...
def update_database_loc(dc_sys_directory: tkinter.StringVar, dc_sys_file_name: tkinter.StringVar,
    ...
    if automatic_names.get():
        ...
        print_log(f"Block Definition file is {Color.default}\"{DATABASE_LOC.block_def}\"{Color.reset}.")
    ...
    for survey_name, survey_info in survey_loc_dict.items():
        ...
        DATABASE_LOC.survey_loc.survey_sheet.append(survey_sheet)
        DATABASE_LOC.survey_loc.all_sheets.append(all_sheets)
        ...
        DATABASE_LOC.survey_loc.start_row.append(start_row)

        ref_col = get_xl_col_from_number_or_letter(survey_info["ref_col"].get())  # Reference Column
        print_log(f"\tCorresponding Reference Column is {Color.default}\"{ref_col}\"{Color.reset}.")
        DATABASE_LOC.survey_loc.ref_col.append(ref_col)

        type_col = get_xl_col_from_number_or_letter(survey_info["type_col"].get())  # Type Column
        print_log(f"\tCorresponding Type Column is {Color.default}\"{type_col}\"{Color.reset}.")
        DATABASE_LOC.survey_loc.type_col.append(type_col)

        track_col = get_xl_col_from_number_or_letter(survey_info["track_col"].get())  # Track Column
        print_log(f"\tCorresponding Track Column is {Color.default}\"{track_col}\"{Color.reset}.")
        DATABASE_LOC.survey_loc.track_col.append(track_col)

        survey_kp_col = get_xl_col_from_number_or_letter(survey_info["survey_kp_col"].get())  # Surveyed KP Column
        print_log(f"\tCorresponding Surveyed KP Column is {Color.default}\"{survey_kp_col}\"{Color.reset}.")
        DATABASE_LOC.survey_loc.survey_kp_col.append(survey_kp_col)

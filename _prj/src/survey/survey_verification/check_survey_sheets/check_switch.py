...
def check_switch(dc_sys_sheet, res_sheet_name: str, survey_info: dict,
    ...
    for object_name, object_value in dc_sys_dict.items():
        original_dc_sys_track, dc_sys_kp = object_value["track"], object_value["kp"]
        dc_sys_track = clean_track_name(original_dc_sys_track, set_of_survey_tracks)

        test_names, sw_type = _get_test_names(object_name)
        ...
        if sw_type == "center":
            survey_object_info, extra_comment = _switch_center_point_to_heel(object_name, survey_object_info, survey_info,
                                                                             sw_type, dc_sys_track)
        else:
            survey_object_info, extra_comment = _switch_heel_point_to_center(object_name, survey_object_info, survey_info,
                                                                             sw_type, dc_sys_track)
...
def _get_test_names(object_name: str) -> tuple[list[str], str]:
    ...
    if object_name.endswith("_L"):
        ...
        test_names.append(object_name.removesuffix("_L") + "_G")
        sw_type = "left"
    elif object_name.endswith("_R"):
        # The right heel name in the survey uses sometimes the French "droite" instead of right
        test_names.append(object_name.removesuffix("_R") + "_D")
        sw_type = "right"
    else:
        # The center point name in the survey is sometimes the switch name only
        test_names.append(object_name.removesuffix("_C"))
        sw_type = "center"
    return test_names, sw_type


def _switch_center_point_to_heel(object_name: str, survey_object_info: dict[str, Union[str, float]],
                                 survey_info: dict[str, dict[str, Union[str, float]]],
                                 sw_type: str, dc_sys_track: str):
    assert sw_type == "center"
    if survey_object_info is not None:  # already associated info in survey
        return survey_object_info, None

    # Try to associate center point to one of its heel if it's on the same track
    left_test_names, _ = _get_test_names(object_name.removesuffix("_C") + "_L")
    ...
    left_survey_object_info = survey_info.get(left_survey_name)

    right_test_names, _ = _get_test_names(object_name.removesuffix("_C") + "_R")
    right_survey_name = test_names_in_survey(right_test_names, dc_sys_track, survey_info, do_print=False,
                                             do_smallest_amount_of_patterns=True)
    right_survey_object_info = survey_info.get(right_survey_name)

    if left_survey_object_info is None and right_survey_object_info is None:
        return None, None
    if left_survey_object_info is not None and right_survey_object_info is not None:
        return None, None  # fail-safe, should not be possible to have both heels on the same track

    if left_survey_object_info is not None:
        survey_object_info = left_survey_object_info
        corresponding_sw_pos = "left"
    else:
        survey_object_info = right_survey_object_info
    ...
    return survey_object_info, extra_comment


def _switch_heel_point_to_center(object_name: str, survey_object_info: dict[str, Union[str, float]],
                                 survey_info: dict[str, dict[str, Union[str, float]]],
                                 sw_type: str, dc_sys_track: str):
    ...
    test_names, _ = _get_test_names(object_name.removesuffix("_L").removesuffix("_R") + "_C")
    survey_name = test_names_in_survey(test_names, dc_sys_track, survey_info)
    survey_object_info = survey_info.get(survey_name)

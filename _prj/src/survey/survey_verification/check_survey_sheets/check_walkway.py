...
def _get_corresponding_ww_survey_extremities(ww_name: str, ww_limits: list[tuple[str, float]],
                                             ...
                                             set_of_survey_tracks: set[str]) -> dict[tuple[str, float], Optional[str]]:
    ...
    if ww_name in plt_dict:
        return get_corresponding_plt_survey_extremities(ww_name, ww_limits, plt_survey_info, set_of_survey_tracks)

    dc_sys_limit_tracks = set([clean_track_name(track, set_of_survey_tracks) for (track, _) in ww_limits])

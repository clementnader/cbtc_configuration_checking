...
def check_object_msgs(object_type, msg_dict: dict, object_name: str, condition: bool, condition_str: str,
                   ...
                   object_type_str: str = None, sig_upstream_ivb: bool = True,
                   zc: str = None, tsr_speed: str = None,
                   tsr_area_missing_speeds: list = None) -> bool:
    ...
    object_class = DCSYS.Flux_PAS_MES if is_flux_pas_mes else DCSYS.Flux_MES_PAS
    ...
    if zc is not None:
        ...
        associated_msgs = {msg_name: msg_info for msg_name, msg_info in associated_msgs.items()
                           if get_dc_sys_value(msg_info, object_class.PasUtilisateur1) == zc}
    ...
    for target_msg_type in target_msg_types:
        ...
        for associated_msg_name, associated_msg_info in associated_msg.items():
            is_msg_vital = get_dc_sys_value(associated_msg_info, object_class.TypeInfo) == VitalOrNotType.SECU
...
def _check_message_zc(object_type, object_name: str, associated_msg, object_type_str, target_msg_type,
                      ...
                      only_one_zc: bool, sig_upstream_ivb: bool) -> bool:
    if not object_name:
        return False
    object_class = DCSYS.Flux_PAS_MES if is_flux_pas_mes else DCSYS.Flux_MES_PAS
    success = True
    ...
    if (not is_flux_pas_mes and (
        ...
        critical_zc, related_critical_obj = get_zc_managing_object(object_type, object_name)

    if only_one_zc:
        expected_zc_list, related_obj = get_zc_managing_object(object_type, object_name, sig_upstream_ivb)
        managed = True  # boolean to tell we are not using all the ZC containing the object but only the one managing it

    msg_zc_dict = dict()
    for associated_msg_name, associated_msg_info in associated_msg.items():
        zc_name = get_dc_sys_value(associated_msg_info, object_class.PasUtilisateur1)
            ...
            if critical_zc is not None:
                if any(zc in missing_zc for zc in critical_zc):
                    ...
                    print_warning(f"A flow of type {Color.yellow}{target_msg_type}{Color.reset} "
                                  f"is supposed to be defined for {object_type_str} "
                                  f"{Color.blue}{object_name}{Color.reset} "

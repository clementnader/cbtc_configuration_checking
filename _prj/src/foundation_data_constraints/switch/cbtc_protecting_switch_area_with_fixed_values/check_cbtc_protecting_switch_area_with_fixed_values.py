...
__all__ = ["check_cbtc_protecting_switch_area_with_fixed_values"]


OUTPUT_DIRECTORY = "."
VERIF_FILE_NAME = "CBTC Protecting Switch Area Verification with fixed distance.xlsx"
FILE_TITLE = "Verification of CBTC Protecting Switch Area with fixed distance"


def check_cbtc_protecting_switch_area_with_fixed_values(train_travel_distance_during_transmission_delay: float,
                                                        moving_part_of_the_switch_distance: float
                                                        ) -> None:
    print_title(f"Verification of CBTC Protecting Switch Area", color=Color.mint_green)

    verif_dict = compute_verif(train_travel_distance_during_transmission_delay, moving_part_of_the_switch_distance)
...
def _update_verif_sheet(ws: xl_ws.Worksheet, start_row: int, verif_dict: dict[str, dict[str, Any]]) -> None:
    ...
    for row, (sw_name, sw_value) in enumerate(verif_dict.items(), start=start_row):
        ...
        moving_part_of_the_switch_distance = sw_value.get("moving_part_of_the_switch_distance")
        dist_to_protect = sw_value.get("dist_to_protect")
        ...
        _add_line_info(ws, row, sw_name, sw_block_locking_area, cbtc_protecting_switch_area,
                       moving_part_of_the_switch_distance, dist_to_protect, list_ivb_to_protect,
                       ivb_that_shall_be_added, ivb_that_can_be_removed)
...
def _add_line_info(ws: xl_ws.Worksheet, row: int, sw_name: str,
                   ...
                   moving_part_of_the_switch_distance, dist_to_protect,
    ...
    create_cell(ws, moving_part_of_the_switch_distance, row=row, column=MOVING_PART_OF_THE_SWITCH_DISTANCE_COL, borders=True,
                align_horizontal=XlAlign.center, nb_of_digits=2)
    # Distance to protect
    create_cell(ws, dist_to_protect, row=row, column=DIST_TO_PROTECT_COL, borders=True,
                align_horizontal=XlAlign.center, nb_of_digits=2)

...
def _check_plt(plt_msg_dict: dict, in_cbtc: bool):
    ...
    for plt_name, plt in plt_dict.items():
        target_msg_types = ([TypeNomLogiqueInfoPASATS.HOLD_NORMAL_DIR,
                            ...
                            if "REGULATION_HOLD_NORMAL_DIR" not in get_class_attr_dict(TypeNomLogiqueInfoPASATS)
                               or "REGULATION_SKIP_NORMAL_DIR" not in get_class_attr_dict(TypeNomLogiqueInfoPASATS)
...
def _check_maz(maz_msg_dict: dict, in_cbtc: bool):
    ...
    for maz_name, maz in maz_dict.items():
        target_msg_types = ([TypeNomLogiqueInfoPASATS.MVT_AUTH,
                            ...
                            if "AM_MVT_AUTH" not in get_class_attr_dict(TypeNomLogiqueInfoPASATS)
...
def _check_message_zc(obj_type, obj_name: str, associated_msg, obj_type_str, target_msg_type,
    ...
    if only_one_zc:
        expected_zc_list, related_obj = get_zc_managing_obj(obj_type, obj_name, plt_end=plt_end)

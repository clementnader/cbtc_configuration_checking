...
def create_verif_file(dc_sys_sheet_name: str, zones_kp_dict: dict):
    file_title = f"KP limits on tracks for {dc_sys_sheet_name} type"
    verif_file_name = f"KP limits on tracks for {dc_sys_sheet_name} type.xlsx"
    ...
    for track, list_min_max_kp in zones_kp_dict.items():
        ...
        for min_kp, max_kp in list_min_max_kp:
            ...
            if ((min_kp, max_kp) == (track_min_kp, track_max_kp)
                ...
                comments = None
            _add_line(ws, row, track, min_kp, max_kp, comments, color_bool)
            row += 1
    ...
    print_success(f"File containing the \"KP Limits for {dc_sys_sheet_name} type\" is available at:\n"
                  f"{Color.blue}{res_file_path}{Color.reset}")
    return res_file_path


def _add_line(ws: xl_ws.Worksheet, row: int, track: str, min_kp: float, max_kp: float,
              comments: Optional[str], color_bool: bool):
    bg_color = XlBgColor.light_blue if color_bool else XlBgColor.light_green
    # Track
    create_cell(ws, track, row=row, column=TRACK_COL, borders=True, bg_color=bg_color)
    # Start KP

...
def create_dc_tu_verif_file(ip_address_dict: dict, ssh_key_dict: dict, tool_version: str) -> str:
    ...
    _create_ip_addr_sheet(wb, ip_address_dict)
    _create_ssh_key_sheet(wb, ssh_key_dict)

    # Save workbook
    c11_d470 = get_c11_d470_version()  # use C11_D470 version to specify it in the verification file name
    if c11_d470:
...
def _create_ip_addr_sheet(wb: openpyxl.workbook.Workbook,
                          ip_addr_dict: dict[int, dict[int, dict[int, dict[str, Union[str, list[tuple[int, str]]]]]]]
                          ) -> None:
    ws, row = create_empty_values_sheet(wb, PMC_IP_ADDRESS_SHEET_NAME, "PMC IP Address",
                                        extra_column="Correspondence with IP Addressing Plan",
                                        set_active=True)
    nb_of_diff_values = 2
    _update_values_sheet(ws, nb_of_diff_values, ip_addr_dict, row)


def _create_ssh_key_sheet(wb: openpyxl.workbook.Workbook,
                          ssh_key_dict: dict[int, dict[int, dict[int, dict[str, Union[str, list[tuple[int, str]]]]]]]
                          ) -> None:
    ws, row = create_empty_values_sheet(wb, PMC_SSH_KEY_SHEET_NAME, "PMC Public SSH Key")
    nb_of_diff_values = 1
    _update_values_sheet(ws, nb_of_diff_values, ssh_key_dict, row)
...
def _manage_status(current_value: str, current_row: int, current_col: int,
    ...
    current_cell = f"{get_xl_column_letter(current_col)}{current_row}"

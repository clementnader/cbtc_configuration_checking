...
def get_number_of_cc(dc_tu_dict: dict[int, dict[int, dict[str, str]]]) -> int:
    ...
    for line_number, sub_dict in dc_tu_dict.items():  # for each line
        for train_unit_number, sub_sub_dict in sub_dict.items():  # for each train unit
            for parameter_name in sub_sub_dict.keys():  # for each parameter
                if CC_ID_REGEX_PATTERN.search(parameter_name) is None:
                    continue
                num_cc = get_num_cc_parameter_name(parameter_name)
                if num_cc == 1:
                    if number_of_cc == 0:
                        number_of_cc = 1
                elif num_cc == 2:
                    ...
                    print_error(f"CC number of {parameter_name} is not 1 or 2\n"
...
def get_ip_address_and_ssh_key(dc_tu_dict: dict[int, dict[int, dict[str, str]]]) -> tuple[
    ...
    for line_number, sub_dict in dc_tu_dict.items():  # for each line
        ...
        for train_unit_number, sub_sub_dict in sub_dict.items():  # for each train unit
            ...
            for parameter_name, parameter_value in sub_sub_dict.items():  # for each parameter
                # CCx_IDx parameter, initialization of the 2 dictionaries
                if CC_ID_REGEX_PATTERN.search(parameter_name) is not None:
                    current_cc_num = get_num_cc_parameter_name(parameter_name)
                    ip_address_dict[line_number][train_unit_number][current_cc_num] = {
                        get_pattern(CC_ID_REGEX_PATTERN): parameter_value,
                ...
                elif CC_PMC_ALPHA_ADDRESS_REGEX_PATTERN.search(parameter_name) is not None:
                    current_cc_num = get_num_cc_parameter_name(parameter_name)
                    current_pmc_num = get_num_pmc_parameter_name(parameter_name)
                    ip_address_dict[line_number][train_unit_number][current_cc_num][
                        get_pattern(CC_PMC_ALPHA_ADDRESS_REGEX_PATTERN)
                    ].append((current_pmc_num, parameter_value))
                # CCx_PMCx_BETA_ADDRESS parameter
                elif CC_PMC_BETA_ADDRESS_REGEX_PATTERN.search(parameter_name) is not None:
                    current_cc_num = get_num_cc_parameter_name(parameter_name)
                    current_pmc_num = get_num_pmc_parameter_name(parameter_name)
                    ip_address_dict[line_number][train_unit_number][current_cc_num][
                ...
                elif CC_PMC_SSH_PUBLIC_KEY_REGEX_PATTERN.search(parameter_name) is not None:
                    current_cc_num = get_num_cc_parameter_name(parameter_name)
                    current_pmc_num = get_num_pmc_parameter_name(parameter_name)
                    ssh_key_dict[line_number][train_unit_number][current_cc_num][

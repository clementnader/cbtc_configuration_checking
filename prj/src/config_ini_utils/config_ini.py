...
def update_cctool_oo():
    ...
    if ini_cctool_oo_schema:
        ...
        DATABASE_LOC.cctool_oo_schema = ini_cctool_oo_schema
    else:
...
def update_config_info():
    ...
    if ini_dc_sys:
        ...
        DATABASE_LOC.dc_sys_addr = ini_dc_sys
    ...
    if ini_kit_c11_dir:
        ...
        DATABASE_LOC.kit_c11_dir = ini_kit_c11_dir

    update_survey_info(config)
    update_apz_ixl_info(config)
    update_control_tables_info(config, ct_type=CONTROL_TABLE_TYPE.route)
    update_control_tables_info(config, ct_type=CONTROL_TABLE_TYPE.overlap)

    print_bar(start="\n")


def update_survey_info(config: configparser.ConfigParser):
    ...
    if ini_survey_address:
        ...
        DATABASE_LOC.survey_loc.survey_addr = ini_survey_address_list
    ...
    if ini_survey_sheet:
        ...
        DATABASE_LOC.survey_loc.survey_sheet = ini_survey_sheet_list
    ...
    if ini_all_sheets:
        ...
        DATABASE_LOC.survey_loc.all_sheets = ini_all_sheets_list
    ...
    if ini_start_row:
        ...
        DATABASE_LOC.survey_loc.start_row = ini_start_row_list

    ini_reference_column = config["survey"].get("reference_column", "").strip()
    if ini_reference_column:
        ini_reference_column_list = [get_xl_col_from_number_or_letter(info.strip())
        ...
        DATABASE_LOC.survey_loc.ref_col = ini_reference_column_list

    ini_type_column = config["survey"].get("type_column", "").strip()
    if ini_type_column:
        ini_type_column_list = [get_xl_col_from_number_or_letter(info.strip())
                                for info in ini_type_column.split(",")]
        print_log(f"\tCorresponding type column is {Color.default}\"{ini_type_column_list}\"{Color.reset}.")
        DATABASE_LOC.survey_loc.type_col = ini_type_column_list

    ini_track_column = config["survey"].get("track_column", "").strip()
    if ini_track_column:
        ini_track_column_list = [get_xl_col_from_number_or_letter(info.strip())
        ...
        DATABASE_LOC.survey_loc.track_col = ini_track_column_list

    ini_surveyed_kp_column = config["survey"].get("surveyed_kp_column", "").strip()
    if ini_surveyed_kp_column:
        ini_surveyed_kp_column_list = [get_xl_col_from_number_or_letter(info.strip())
        ...
        DATABASE_LOC.survey_loc.survey_kp_col = ini_surveyed_kp_column_list

    list_nb_survey_info = [len(info) for info in (DATABASE_LOC.survey_loc.survey_addr,
                                                  DATABASE_LOC.survey_loc.survey_sheet,
                                                  DATABASE_LOC.survey_loc.all_sheets,
                                                  DATABASE_LOC.survey_loc.start_row,
                                                  DATABASE_LOC.survey_loc.ref_col,
                                                  DATABASE_LOC.survey_loc.type_col,
                                                  DATABASE_LOC.survey_loc.track_col,
                                                  DATABASE_LOC.survey_loc.survey_kp_col)]
...
def update_apz_ixl_info(config: configparser.ConfigParser):
    ...
    if ini_ixl_apz_address:
        ...
        DATABASE_LOC.ixl_apz.ixl_apz_file = ini_ixl_apz_address
    ...
    if ini_ixl_apz_sheet_name:
        ...
        DATABASE_LOC.ixl_apz.ixl_apz_sheet_name = ini_ixl_apz_sheet_name
...
def update_control_tables_info(config: configparser.ConfigParser, ct_type: str):
    control_tables_type = f"{ct_type.lower()}_control_tables"
    database_ct_loc = (DATABASE_LOC.control_tables_route if ct_type == CONTROL_TABLE_TYPE.route
                       else DATABASE_LOC.control_tables_overlap)

...
def load_survey() -> dict:
    ...
    if not LOADED_SURVEY:
        for survey_addr, survey_sheet, start_line, ref_col, type_col, track_col, survey_kp_col in \
                get_d932_loc_info():
            ...
            LOADED_SURVEY.update(get_survey(LOADED_SURVEY, d932_sh,
                                            start_line, ref_col, type_col, track_col, survey_kp_col,
...
def get_d932_loc_info():
    ...
    return zip(survey_addr, survey_sheet, start_line, ref_col, type_col, track_col, survey_kp_col)


def get_survey(loaded_survey, d932_sh, start_line, ref_col, type_col, track_col, survey_kp_col,
    ...
    for line in range(start_line, get_xl_ws_number_of_rows(d932_sh) + 1):
        ...
        if obj_name in loaded_survey[survey_type]:
            ...
            if old_surveyed_kp is not None and surveyed_kp is not None and surveyed_kp != old_surveyed_kp:
                # TODO write this info inside the comments column
            ...
            loaded_survey[survey_type][obj_name].update({"surveyed_kp_comment": surveyed_kp_comment})

...
def get_joints_dict():
    ...
    for block_name, block_val in block_dict.items():
        ...
        for limit_position, matching_block_name in matching_blocks.items():
            joint_name, joint_name2 = _get_corresponding_joint(block_name, matching_block_name)
...
def _get_corresponding_joint(block_name: str, matching_block_name: str):
...
def _get_joint_name(block_name: str, matching_block_name: str):
    return "JOI_" + block_name.removeprefix("TC_") + _get_joint_suffix_name(block_name, matching_block_name)
...
def _get_joint_suffix_name(block1_name: str, block2_name: str):
    split_diff_start = -1
    split1 = block1_name.split("_")[1:]
    split2 = block2_name.split("_")[1:]

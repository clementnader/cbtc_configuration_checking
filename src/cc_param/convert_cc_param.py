...
def convert_ccparam():
    parser = argparse.ArgumentParser(
        add_help=True,
        description=f"Convert CCParameter.csv files in kit {Color.white}C11_D470{Color.reset} from Unix to Windows.",
    )
    parser.add_argument("-i", "--input_kit_c11_dir", dest="input_dir", action="store", type=str, required=True,
                        ...
                        help=f"path to the kit C11_D470 folder where the TrainUnit files are stored, "
                             ...
                             f"{Color.dark_grey}(if the path contains spaces, put it between quotes){Color.reset}")
    parser.add_argument("-d", "--destination_folder", dest="dest_dir", action="store", type=str, required=False,
                        metavar=f"{Color.blue}Destination_Root_Folder{Color.reset}",
                        default=DEFAULT_RES_DIRECTORY,
                        help=f"root folder where the result converted directory will be stored, "
                             f"default is the Desktop ({Color.dark_green}{DEFAULT_RES_DIRECTORY}{Color.reset})")
...
def copy_kit_c11_d470(input_dir, dest_dir):
    ...
    print(f"\nConverting the CCparameters.csv files from:\n"
          f"\t{Color.dark_cyan}{input_dir}{Color.reset}\n"
          f"to\n"
          f"\t{Color.dark_cyan}{full_res_dir}{Color.reset}\n")
    create_success = create_result_dir(full_res_dir)
    if not create_success:
        print(f"{Color.red}Process aborted{Color.reset}\n")
        return
    ...
    print(f"\nConversion is done, the result folder can be found at:\n"
          f"\t{Color.green}{full_res_dir}{Color.reset}")
...
def create_result_dir(full_res_dir):
    if os.path.exists(full_res_dir):
        print_warning(f"The result folder already exists"
                      f"\n at {full_res_dir}.")
        if input(f"{Color.white}Are you sure you want to overwrite it?{Color.reset} "
                 f"{Color.dark_cyan}[y/n]{Color.reset} ").capitalize() not in ["Y", "YES"]:
            return False

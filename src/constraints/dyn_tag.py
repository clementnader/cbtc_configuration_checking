...
def r_dyntag_3():
    ...
    print_log(f"\r{progress_bar(nb_dyn_tags, nb_dyn_tags, end=True)} processing DMC timeout distance "
              f"of dynamic tags finished.\n")
...
def get_dist_dyn_tag_to_joint(dyn_tag, ivb):
    ...
    for i, (seg, x) in enumerate(get_dc_sys_zip_values(ivb_dict[ivb], DCSYS.IVB.Limit.Seg, DCSYS.IVB.Limit.X), start=1):
        if (downstream and is_seg_downstream(dyn_tag_seg, seg, dyn_tag_x, x)) \
                or (not downstream and is_seg_downstream(seg, dyn_tag_seg, x, dyn_tag_x)):
            dist = get_dist(dyn_tag_seg, dyn_tag_x, seg, x) if downstream \
                else get_dist_downstream(seg, x, dyn_tag_seg, dyn_tag_x)

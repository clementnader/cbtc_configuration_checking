    ...
    for line in range(START_LINE, sh.nrows + 1):
        ...
        if obj_name in sh_dict:
            print_error(f"Two objects have the same name {Color.beige}{obj_name}{Color.reset} "
                        f"in sheet {Color.blue}{sh.name}{Color.reset}.")
...
def get_generic_sh_dict(sh: xlrd.sheet, columns_dict: dict[str], generic_obj_name: dict) -> dict:
    ...
    sh_dict = {f"{sh.name}_{line+1}": val for line, val in enumerate(temp_dict.values())}
    return sh_dict
...
def get_info_for_object(sh: xlrd.sheet, line: int, columns_dict: dict[str]):
    ...
    for key, column in columns_dict.items():
        if isinstance(column, int):
            ...
            check_value_has_spaces(sh.name, value, line, column, key)
    ...
    if isinstance(value, str) and value.startswith(" "):
        if not any(value.startswith(old_val) for old_val in g_list_of_values_with_leading_spaces):
            print_warning(f"There is a leading space in sheet {Color.yellow}{sh_name}{Color.reset} "
                          f"at cell {Color.blue}{get_xl_column(column)}{line}{Color.reset}:\n\t"
